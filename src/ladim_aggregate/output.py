...
class MultiParquet:
    ...
    def flush(self):
        data = pd.concat(self._cache, ignore_index=True)
        fname = self._pattern.format(self._num_files_written)
        data.to_parquet(fname, compression='snappy')
        self._num_files_written += 1
        self._cache = []


def replace_first_qblock(text: str) -> str:
    """Replaces expression containing question marks with format string.

    For instance, an expression of form 'dsf_???_sdf' is replaced with 'dsf_{:03d}_sdf'
    """
    import re
    def repl(match):
